function NewPlayer(x,y,color,right,left,jump,dash)
    ...
    Player.RunForce        = 4000 -- force appliquée quant le joueur avance
    Player.Direction       = 1
...
--########################################################################################################################################
    ...
    function Player.Animate (dt)
        ...
        Player.ScaleX = Player.ScaleX * Player.Direction * Player.ScaleX/math.abs(Player.ScaleX) --inverse le sprite quand le personnage se retourne
    ...
    --Turn around
        if Player.SpeedX/math.abs(Player.SpeedX) * Player.AccelerationX < 0 and Player.Grounded and Player.CurrentAnimation ~=Player.Animations.Idle and Player.CurrentAnimation ~= Player.Animations.HiddenIdle then -- si joueur freine alors TurnAround
            Player.CurrentAnimation = Player.Animations.TurnAround
            Player.ScaleX = Player.ScaleX * Player.SpeedX/math.abs(Player.SpeedX) * Player.Direction
            Player.AnimationsReset()
        end
    -- Idle
...
--########################################################################################################################################
    ...
    function Player.Movments ()
        if love.keyboard.isDown(Player.Controls.Right)  then --droite
            if Player.Grounded then
                Player.ApplyForce(Player.RunForce,0)
                
            else
                Player.ApplyForce(Player.RunForce/Player.AirControl,0)
            end
            Player.Direction = 1
        end
    ...
    function Player.keypressed(key)   --Jump  
        if key == Player.Controls.Jump  then
            if Player.Grounded then
                Player.ApplyForce(0,-Player.JumpForce)
            elseif Player.DoubleJumpCount < Player.TotalDoubleJumps then --double Jump
                Player.ApplyForce(0,-Player.DoubleJumpForce)
                Player.SpeedY = 0
                Player.DoubleJumpCount = Player.DoubleJumpCount + 1
            end
        end
        if key == Player.Controls.Dash and Player.DashCount < Player.TotalDash then -- Dash
            Player.ApplyForce(Player.DashForce* Player.Direction,0)
                ...
                if  i == Player.Xmap or j == Player.Ymap then
                    if  Map[i][j].Occupied then
                        ...
                        if Player.Ymap > j and Player.Y - Player.Height/2 < Map[i][j].Y + Map[0][0].Size then
                            ...
                            Player.SpeedY = 0
                        elseif Player.Ymap < j and Player.Y + Player.Height/2 > Map[i][j].Y  and Player.SpeedY > 0 then
